...
namespace SenserModels.Common
{
	...
	public abstract class PageTemplate
	{
		...
		public virtual string GetTemplate(string forumpath,string skinName, string templateName, int nest,int templateid)
		{
            ...
            string inherits = "CnTaxLawyer.Web." + templateName;
            //如果指定风格的config模板文件存在
            ...
            using (System.IO.StreamReader objReader = new System.IO.StreamReader(createFilePath, Encoding.UTF8))
			{
				...
				if (nest == 1)
				{
                    ...
                    if ("\"".Equals(inherits))
                    {
                        inherits = "CnTaxLawyer.Forum.PageBase";
                    }
			...
			if (nest == 1)
			{
                ...
                string template = string.Format("<%@ Page language=\"c#\" AutoEventWireup=\"false\" EnableViewState=\"false\" Inherits=\"{0}\" %>\r\n<%@ Import namespace=\"System.Data\" %>\r\n<%@ Import namespace=\"SenserModels.Common\" %>\r\n<%@ Import namespace=\"CnTaxLawyer.Forum\" %>\r\n<%@ Import namespace=\"CnTaxLawyer.Entity\" %>\r\n{1}\r\n<script runat=\"server\">\r\noverride protected void OnInit(EventArgs e)\r\n{{\r\n\r\n\t/* \r\n\t\tThis page was created by CnTaxLawyer Template Engine at {2}.\r\n\t\t本页面代码由CnTaxLawyer模板引擎生成于 {2}. \r\n\t*/\r\n\r\n\tbase.OnInit(e);\r\n{3}\r\n\tResponse.Write(templateBuilder.ToString());\r\n}}\r\n</script>\r\n", inherits, extNamespace, DateTime.Now.ToString(), strReturn.ToString());
		...
		private string ConvertTags(int nest,string forumpath, string skinName, string inputStr, int templateid)
		{
			...
			foreach (Match m in r[12].Matches(strTemplate))
			{
				if (IsCodeLine)
				{
					strTemplate = strTemplate.Replace(m.Groups[0].ToString(), "YTXRequest.GetString(\"" + m.Groups[2].ToString() + "\")");
				}
				else
				{
					strTemplate = strTemplate.Replace(m.Groups[0].ToString(), string.Format("\" + YTXRequest.GetString(\"{0}\") + \"", m.Groups[2].ToString()));
				}
