...
namespace SerialPortController
{
    public class DataCollector : Senser
    {
        ...
        public DataCollector(byte address, CollecterComController comController)
        {
            Reset();

            this.comController = comController;

            ((CollecterComController)this.comController).OfflineEvent += new EventHandler<OnlineEventArgs>(comController_OfflineEvent);
        ...
        public bool SetSamplingInterval(int samplingInterval)
        {
            ...
            if (((CollecterComController)this.comController).SendReturnBool(GenerateAddress(SAMPLING_INTERVAL),
                 WRITE_SINGLE_VALUE, WORD, data))
            {
                //Trace.WriteLine("SAMPLING_INTERVAL");
                if (((CollecterComController)this.comController).SendReturnBool(GenerateAddress(SAMPLING_INTERVAL_SIGN),
        ...
        public bool SetMaxValue(int value)
        {
            ...
            if (((CollecterComController)this.comController).SendReturnBool(GenerateAddress(MAX_VALUE),
                 WRITE_SINGLE_VALUE, WORD, data))
            {
                return true;
            }
        ...
        {
            byte[] data = GenerateWord(value);
            if (((CollecterComController)this.comController).SendReturnBool(GenerateAddress(MIN_VALUE),
                 WRITE_SINGLE_VALUE, WORD, data))
            {
                return true;
            }
        ...
        {
            //byte[] data = GenerateWord(value);
            if (((CollecterComController)this.comController).SendReturnBool(GenerateAddress(DEVICE_RANGE_SIGN),
                 WRITE_SINGLE_VALUE, WORD, new byte[] { 0x00, 0x01 }))
            {
                byte[] values = ((CollecterComController)this.comController).SendReturnData(GenerateAddress(CURRENT_VALUE),
                READ_VAULES, WORD, null);
        ...
        public bool SetSystemTime()
        {
            ...
            if (((CollecterComController)this.comController).SendReturnBool(GenerateAddress(SYSTEM_TIME),
                 WRITE_MUTI_VALUE, DOOUBLE_WORD, data))
            {
                //Trace.WriteLine("SYSTEM_TIME");
                if (((CollecterComController)this.comController).SendReturnBool(GenerateAddress(TIME_SYNC_SIGN),
                 WRITE_SINGLE_VALUE, WORD, new byte[] { 0x00, 0x01 }))
                {
        ...
        {
            bool onwork = ((CollecterComController)this.comController).SendReturnBool(GenerateAddress(SAMPLING_START_SIGN),
                 WRITE_SINGLE_VALUE, WORD, new byte[] { 0x00, 0x01 });
            //Trace.WriteLine("SAMPLING_START_SIGN");
            if (onwork)
            {
        ...
        {
            byte[] counts = ((CollecterComController)this.comController).SendReturnData(GenerateAddress(RECORD_COUNT),
                READ_VAULES, WORD, null);
            //Trace.WriteLine("GetRecordCount");
            if (counts != null)
            {
        ...
        {
            byte[] types = ((CollecterComController)this.comController).SendReturnData(GenerateAddress(DEVICE_TYPE),
        ...
        public bool SetDeviceRange(RangeSaveType range)
        {
            ...
            if (((CollecterComController)this.comController).SendReturnBool(GenerateAddress(DEVICE_RANGE),
                WRITE_SINGLE_VALUE, WORD, data))
            {
                data = GenerateWord(range.SecondValue);
                if (((CollecterComController)this.comController).SendReturnBool(GenerateAddress(DEVICE_RANGE + 2),
                WRITE_SINGLE_VALUE, WORD, data))
                {
        ...
        {
            byte[] length = ((CollecterComController)this.comController).SendReturnData(GenerateAddress(SINGLE_RECORD_LENGTH),
                READ_VAULES, WORD, null);
        ...
        public byte[] ReadLastRecord()
        {
            ...
            byte[] data = ((CollecterComController)this.comController).SendReturnData(GenerateLastOffset(),
        ...
        public byte[] ReadRealTimeRecord()
        {
            ...
            while (this.recordsRead < recordCount)
            {
                ...
                if (countToRead != 0)
                {
                    ...
                    byte[] data = ((CollecterComController)this.comController).SendReturnData(GenerateOffset(),
