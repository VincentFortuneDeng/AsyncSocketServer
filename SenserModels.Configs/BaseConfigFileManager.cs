...
namespace SenserModels.Config
{
    ...
    /// 基本设置信息管理类
    /// </summary>
    class BaseConfigFileManager : SenserModels.Config.DefaultConfigFileManager
    {
        ...
        public new static string ConfigFilePath
        {
            get
            {
                if (filename == null)
                {
                    ...
                    if (context != null)
                    {
                        filename = context.Server.MapPath("~/SenserModelsApp.config");
                    ...
                    {
                        filename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "SenserModelsApp.config");
                    }

                    if (!File.Exists(filename))
                    {
                        throw new YTXException("发生错误: 虚拟目录或网站根目录下没有正确的SenserModelsApp.config文件");
                    }
                }
                return filename;
            }
