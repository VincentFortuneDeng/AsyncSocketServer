...
namespace AsyncSockets
{
    ...
    public class AsyncUserToken:EventArgs
    {
        ...
        string m_connectionId;//内部连接ID
        IPEndPoint m_endPoint;//终结点
        byte[] m_receiveBuffer;//缓冲区
        int m_count;
        int m_offset;//偏移量
        int m_bytesReceived;//已经接收到的字节数
        SocketAsyncEventArgs m_readEventArgs;// SocketAsyncEventArgs读对象
        object m_operation;
