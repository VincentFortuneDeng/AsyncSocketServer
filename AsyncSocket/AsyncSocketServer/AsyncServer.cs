...
namespace AsyncSockets.AsyncSocketServer
{
    /// <summary>
    /// 异步Socket通讯服务器类
    /// </summary>
    public class AsyncServer
    {
        ...
        /// 客户端已经连接事件
        /// </summary>
        public event EventHandler<AsyncUserToken> OnConnected;        // 客户端已经连接事件        
        /// <summary>
        /// 客户端错误事件
        /// </summary>
        public event EventHandler<AsyncSocketErrorEventArgs> OnError;     // 客户端错误事件       
        /// <summary>
        /// 接收到数据事件
        /// </summary>
        public event EventHandler<AsyncUserToken> OnDataReceived;           // 接收到数据事件
        /// <summary>
        /// 数据发送完成
        /// </summary>
        ...
        /// 客户端断开连接事件
        /// </summary>
        public event EventHandler<AsyncUserToken> OnDisconnected;     // 客户端断开连接事件
        /// <summary>
        /// 客户端是否在线
        /// </summary>
        ...
        public AsyncServer(int numConnections, int BufferSize)//构造函数
        {
            // 重置接收和发送字节总数
            m_totalBytesRead = 0;
            m_totalBytesWrite = 0;
        ...
        private void ProcessAccept(SocketAsyncEventArgs e)
        {
            if (e.AcceptSocket.Connected)
            {
                ...
                EventHandler<AsyncUserToken> handler = OnConnected;
                // 如果订户事件将为空(null)
                if (handler != null)
                {
        ...
        private void ProcessReceive(SocketAsyncEventArgs e)
        {
            ...
            if (e.BytesTransferred > 0 && e.SocketError == SocketError.Success)
            {
                ...
                EventHandler<AsyncUserToken> handler = OnDataReceived;
                // 如果订户事件将为空(null)
                if (handler != null)
                {
            ...
            if (null != token)
            {
                lock (((ICollection)this.m_tokens).SyncRoot)
                {
                    if (this.m_tokens.ContainsValue(token))
                    {
                        ...
                        EventHandler<AsyncUserToken> handler = OnDisconnected;
                        // 如果订户事件将为空(null)
        ...
        {
            EventHandler<AsyncSocketErrorEventArgs> handler = OnError;
            // 如果订户事件将为空(null)
            if (handler != null)
            {
                if (null != token)
